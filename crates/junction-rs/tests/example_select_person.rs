...
#[test]
fn example_select_person_runs_and_prints_selected() {
    let exe = find_example_bin("select_person").expect(
        "example binary not found; run with `cargo test --all-targets -p junction-rs` to build examples",
    );
    let out = Command::new(exe)
        .output()
        .expect("failed to run select_person example");
    ...
    assert!(
        stdout.contains("Selected 1 people"),
        "missing selected count in output: {}",
        stdout
    );
    assert!(
        stdout.contains("Tom"),
    ...
    assert!(
        ...
        "unexpected 'Jaime' in filtered output: {}",
        stdout
    );
}
